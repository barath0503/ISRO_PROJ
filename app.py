...
def print_node_timings(snapshot: "Snapshot", gnss_on: bool = False) -> None:
    """Print a formatted table of node timings to the terminal."""
    global _step_counter, _ansi_enabled
    _step_counter += 1
    if _step_counter % TERMINAL_PRINT_INTERVAL != 0:
        return

    # Enable ANSI on Windows (once)
    if sys.platform == "win32" and not _ansi_enabled:
        os.system("")
        _ansi_enabled = True
    ...
    log.info(f"\n{ANSI_CYAN}{ANSI_BOLD}{'=' * w}{ANSI_RESET}")
    log.info(
    ...
    log.info(f"{ANSI_CYAN}{'-' * w}{ANSI_RESET}")
    log.info(
    ...
    log.info(f"{ANSI_DIM}{'-' * w}{ANSI_RESET}")
    ...
    log.info(
        ...
        f"  {ANSI_DIM}|{ANSI_RESET}  "
        f"{ANSI_MAGENTA}PINN Gain: {gain:>8.3f} ns{ANSI_RESET}"
        f"  {ANSI_DIM}|{ANSI_RESET}  "
        f"{order_color}Order R: {order:.4f}{ANSI_RESET}"
    )
    log.info(f"{ANSI_CYAN}{ANSI_BOLD}{'=' * w}{ANSI_RESET}")
...
class ClockSimulation:
    ...
    def _pinn_correction(self, drift_gain: float) -> np.ndarray:
        physics_drift = 0.94 * self._drift_rate(drift_gain)
        holdover_anchor = 0.38 * np.tanh(self.theta / 0.55)
        consensus_error = 0.08 * (self.theta - np.mean(self.theta))
